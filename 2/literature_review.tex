...
%Although the function $g$ can be, and often is, a monochromatic sine wave, it does not have to be. 
...
%Thus based on the theory of Fourier decomposition, a real wave must consist of the superposition of an infinite set of sinusoidal frequencies.
In addition to the relations above, the wave vector and the angular frequency are not independent; they must adhere to the dispersion relation:
\begin{equation}
...
%Since the magnetic field is dependent on the electric field, 
Thus it is sufficient to just consider electric field, which equals:
\begin{equation}
\vec{E}(\vec{r}) = \vec{E^0} e^{-i \vec{k} \cdot \vec{r}} %= \vec{E^0} e^{-ikz}
\end{equation}
...
\vec{E}(\vec{r},t) 
	...
	= \vec{E^0} \cos(wt - \vec{k} \cdot \vec{r}) %\vec{E^0} \cos{ \left( wt - kz + \phi_0 \right)}
\end{equation}

If we define the $z$ direction as the travelling direction of the EM wave,
%	and x direction is the direction of $\vec{E}$,
%	then by Faraday's Law, the magentic field $\vec{B}$ lies in y direction, 
%		and is related to the electric vector by $c^2 \frac{\partial B}{\partial z} = \frac{\partial E}{\partial t}$
...
When the two orthogonal components have exactly the same amplitude and are exactly ninety degrees out of phase, i.e. $E_x^0=E_y^0$ and $ | \phi_x^0 - \phi_y^0 | = \pi/2$. 
%There are two possible phase relationships that satisfy this requirement: the x component can be ninety degrees ahead of the y  component or it can be ninety degrees behind the y component. 
...
%The choice of basis vector used to represent horizontal and vertical directions are evidently arbitrary.
%In general, any pair of orthogonal polarization states can be used.
%Formally orthogonal vector pair is defined as one having a zero inner product.
%Another common choice is left and right circular.
%In anyway, implicit in the representations above is the orientation of the coordination frame.
%Of the two angles frequently used in polarization ellipse analysis, one $\Psi$ is dependent on the coordination frame and the other may not $\chi$

%To recap $\chi = \tan^{-1} (1/\epsilon)$, with $\epsilon$ being the ellipse's ellipticity.
%In polarization ellipse, $\epsilon=1$ indicates circular polarization, while $\epsilon=0$ or $\epsilon=\infty$ corresponds to linear polarizations.
%An alternative to ellipticity angle $\chi$ is the eccentricity angle.
%However, the use of ecccentricity angle has not been widely used, as it has no obvious geometrical interpretation in Poincare Spheres.
...
%Then any arbitrary state, written in R-L basis is given by:
%\begin{equation}
%| \Psi \rangle 
%= 
%a_R e^{i \phi_x - i \theta} | R \rangle 
%+ 
%a_L e^{i \phi_x + i \theta} | L \rangle 
%\end{equation}
...
\subsection{Different Modes of Polarimetric SAR}
...
%The famous Lee, Kuan, and Frost ﬁlters in the ﬁrst category provide MMSE reconstructions based on measured local statistics.
%In the second category, diﬀerent scene distributions are used: Gaussian, Gamma, and parameter-based distributions.
...
\subsection{The Multivariate Nature and The Observables in POLSAR data}
...
\section{Existing Measures of Distance for (POL)SAR data and their applications}
