...
%  which includes several statistically consistent discrimination measures. %can be derived.
...
%Specifically POLSAR data have been statistically modelled as following the complex Wishart distribution.
...
The most obvious are the Bhattacharyya and Bartlett distances. % are easily shown to be related.
...
%We will now use 
This is used to develop a generic scalar statistical model. %log-transformed measures of distance.
...
Eqn. \ref{eqn:determinant_distribution} %and \ref{eqn:log_determinant_distribution}
  , gives us:
\vspace{-2mm}  
%{
%\begin{align}
...
For the more matured SAR field, 
...
For the less matured POLSAR field the same case should apply, 
  except that so far only asymptotic distributions have been derived for the most common foundation: likelihood test statistics \citep{Conradsen_2003_TGRS_4}.

With the insight gained from section \ref{sec:sar_special_case_of_polsar}, this section presents a few new results.
...
First, similar to the way that the statistical SAR intensity models have been used as a foundation for SAR discrimination measures, e.g. intensity-ratio \citep{Rignot_1993_TGRS_896},
  the proposed statistical model for POLSAR covariance matrix determinant can also be viewed as providing a foundation for POLSAR discrimination measures, i.e. the likelihood test statistics.
...
As for the first matter, in view of the models given in Eqn \ref{eqn:determinant_distribution},
  the likelihood test statistics presented in \citet{Conradsen_2003_TGRS_4} and rewritten in Eqns \ref{eqn:ori_likelyhood_stats} \& \ref{eqn:log_likelyhood_stats}
can be expressed as
...
This, in essence, derives an exact statistical distribution for the likelihood test statistics,
  as opposed to the asymptotic distribution derived in \citet{{Conradsen_2003_TGRS_4}}.
...
They are hence multi-dimensional extensions of the widely used SAR intensity-ratio discrimination measure.
...
Histograms are then plotted for all of the proposed observables %both the intensity and intensity ratio %the log-distance and contrast
...
In all cases, the good visual match between the actual data and the respective model apparently validates the proposed model. %providing us a simple validation of the result.
...
And the actual distribution is plotted against the model yielding an obvious visual match in  Fig. \ref{fig:verify_det_ratio_model_2D} which shows the $d=2$ plots for determinant, determinant ratio and change ratio for both datasets. %It is very clear that the model histogram matches the data well, in fact appearing similar to a smoothed data response. Evidently the match is good for $d=2$.
...
%  due to the relation between the determinant and the eigen-values  
