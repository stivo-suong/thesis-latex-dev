...
SAR speckle phenomena are explained as the interference of many coherent but dephased backscattering components;
  each reflecting from different and distributed elementary scatterers \citep{Oliver_ProcIEEE_1963, Leith_ProcIEEE_1971}. 
...
%***IVM: Hai your citations are wrong in this paragraph, they should be \citep (I think I've fixed them all though)
Early works from the radar community had focused on the theoretical introduction of the scattering matrix such as the Sinclair matrix or Mueller matrices, for full, i.e. quad, polarimetric measurements \citep{Sinclair_1950_ProcsIRE}.
%(Sinclair, 1950), (TODO) 
These works were continued by Boerner, and the first polarimetric images were captured by NASA JPL at Caltech in 1991 \citep{Zebker_1991_ProcsIEEE}.

Systems were then engineered and commercial POLSAR satellites, e.g. RadarSat, TerraSAR, Alos-PalSAR were launched.
However due to operational restrictions on payload, power and data-downlink (speed?), the satellites also offer dual-channel polarimetric SAR modes as a compromise between theoretical polarimetric requirements and practical limitations.
Initially, to the end user, while dual-channel partial polarimetry may not provide the complete polarimetric signature of the target, the loss is compensated for by either better resolutions or higher coverage and lower cost.  (Comment - this sentence is not clear to me. How does the ëlossí be compensated by ëbetter resolutionsí? I thought loss means the resolution becomes worse to start with)

Until very recently, these partial polarimetry modes were not formally taken up by scientists and academia. 
In his pioneer paper in 2005, Souyris\citep{Souyris_2005_TGRS} introduced the so-called compact polarimetry mode and suggested that it may be possible to reconstruct full polarimetric information from compact polarimetric data.
Raney\citep{Raney_2006_IGARSS} also showed that dual-channel polarimetry is not only capable of measuring the complete polarization information of the back-scattered EM wave (via Stokes parameters), it is also capable of detecting the basic target's polarimetric characteristic, e.g. odd or even bounce phenomena.
...
\citet{Raney_2006_IGARSS} demonstrated a different perspective by proposing a hybrid architecture.
...
(Comment: check the presentation of this equation in the pdf file)
...
\right) 
\end{equation}
In \citep{Raney_2007_IGARSS}, Raney shows that hybrid circular modes bring many conveniences to hardware calibration.
Also in a follow up study \citep{Raney_2007_TGRS}, he demonstrated the classification power of hybrid polarimetry.
Furthermore, Lardeux \citep{Lardeux_2007_ESASP} found that for target classification using SVM techniques, compact polarimetry mode shows comparable results to full polarimetric data.
Notably, the hybrid architectural concept has been adopted and brought towards implementation by NASA's upcoming Desdynl satellite \citep{Raney_2009_RadarConf}.
...
Souyris, in 2005, published a paper \citep{Souyris_2005_TGRS} which claimed and presented an algorithm to reconstruct full polarimetric information from the given partial compact polarimetric data.
The algorithm is claimed to work on natural surfaces.
In \citep{Nord_2009}, Nord reviewed the reconstruction problem for various different partial polarimetry modes, and an improved algorithm is given. %****IVM: Hai are Nord / Ainsworth single author? If not, you need an "et. al."
In \citep{Ainsworth_2009_ISPRS}, Ainsworth reviewed performance of target classification on different partial modes, reconstructed pseudo-full mode and full polarimetric data.
...
(Comment: the following should be re-aligned into multiple equations, instead of one long sentence ñ see pdf file) 
...
(comment: you should link the following paragraph to the above discussion ñ I suggest move the following paragraph to earlier as shown here)  
...
The matrix is normally called the Sinclair matrix in SAR literature, as backward scattering alignment is normally adopted for SAR \citep{Sinclair_1950_ProcsIRE}.
In the case of mono-static SAR, i.e. both radar transmitter and receiver can be considered as located in a single place, physical reciprocity leads to $S_{HV}=S_{VH}$ \citep{Nghiem_1992_RadioSci}.
...
\end{equation}
%***IVM: Hai, maybe you can use the small maths font for this matrix... I can't remember the exact syntax, but it's something simple like \small or \mathsmall or similar....


(comment: check layout of the above equation in the pdf file) 
...
Equivalent matrices with all real value entries which carry the same information are called the Mueller and the Kennaugh matrix, which are to be used in forward and backward scattering alignment respectively\citep{Guissard_1994_TGRS}. 
...
Boerner \citep{Boerner_1991_ProcsIEEE} has given $U_2$ as:
\begin{equation}
...
where
	...
	and is required (which one?) for the determination of the initial phase of the Jones vector in the new reference basis.

(comment: the above senetence is not clear, are you referring to the ratio or the phase reference, or both?)


In the same vein, Ferro-Famil, 2000 gives%****IVM pls change this to a proper \citep{}
\begin{equation}
U_2 = \left(
\begin{array}{c c}
 \cos(\Psi) & -\sin(\Psi) \\
...
The results here, together with Stokes Vector's formula, can be used to prove the following identity, which form the basis of the approach used throughout this thesis:
\begin{equation}
S_t = 
\left(
	\begin{array} {c}
...
In \citep{Alberga_2008_IJRS_4129}, the performance of different scalar POLSAR observables for classification purposes are evaluated.
...
For full-pol (comment ñ do you mean full-polarization? If so, should write the complete word. See further below on this) the covariance matrix has the form of
\begin{equation*}
C_3 = 
\begin{bmatrix}
...
\end{bmatrix}
\end{equation*}
For part-pol(comment - also here may be you want to explains thesesterm first befpore using it in the text??), it reduces to either (??comment- there is no ëorí in the following equation ñ do yo mean either of the two following equations, or both ñ then no ëeitherí?):
\begin{equation*}
  C_{2,h} =
  \begin{bmatrix}
    S_{hh}^2 & S_{hh}S_{hv}^* \\
...
\begin{enumerate}
\item the polarization ellipse parameters \citep{Barakat_1985_IJOptics}: i.e.
  \begin{enumerate}
  ...
  \end{enumerate}
\item the Stokes parameters \citep{Eliyahu_1993_PhysRevE_2881}: i.e.
  \begin{enumerate}
  ...
  \item $S_3 = \Im (C_{12})$
  \end{enumerate}
\item and the normalized Stokes parameters $s_i=S_i/S_0, i>0$ \citep{Brosseau_1995_AppliedOptics_4788}
\end{enumerate}
...
Specifically the following observables were studied:  
  ...
  \item co-pol phase difference $\phi_{VV/HH} = arg(S_{VV}S_{HH}^*) $ \citep{Joughin_1994_TGRS_562,Lee_1994_TGRS_1017},
...
Furthermore, other studies have offered
  ...
  as well as the largest eigen-value of the covariance matrix $\lambda_1$ \citep{Erten_2012_Sensors_2766}.
...
%The famous Lee, Kuan, and Frost Ô¨Ålters in the Ô¨Årst category provide MMSE reconstructions based on measured local statistics.
%In the second category, diÔ¨Äerent scene distributions are used: Gaussian, Gamma, and parameter-based distributions.

In the first category the speckle random process is assumed to be stationary over the whole image. 
Then speckle models are proposed, such as (?)the multiplicative speckle model is first approximated by a linear model. A speckle reduction filter is formulated  (Comment: This paragraph is not clear.)
as follows: 
\begin{equation}
...
For the Lee filter \citep{Lee_PAMI_1980}, the weight function is proposed to be:
\begin{equation}
...
In the approach of the Kuan filter \citep{Kuan_1985_PAMI}, the multiplicative speckle model is first transformed into a single-dependent additive noise model, and then the MMSE criterion is applied. 
...
The Frost filter \citep{Frost_PAMI_1982} is different from the Lee and Kuan filters, where the scene reflectivity is estimated by convolving the observed image with the impulse response of the SAR system. 
...
Our literature search yielded only a single article \citep{Amirmazlaghani_2009_TIP} tackling SAR data's heteroskedastic feature, suggesting a lack of appropriate attention within the research community about such characteristic and its effects.
...
Under such conditions, Ordinary Least Square methods may not provide optimal results \citep{Woods_PAMI_1984}. 
In fact, as is shown by the mathematical formula above (which one?), weighted average formulae were being used.
...
However, in SAR data, this variance is not available directly and can only be estimated (note the concept of variation coefficient). (comment: this is mentioned the 1st time in the text?)
Our analysis will later show that estimating variance is as hard as estimating the un-speckled signal itself. However, it will also be shown 
...
In the second approach, (comment: it is not clear what is the first approach, so far there seems to be many approaches discussed already) the filters assume that speckle is not stationary globally but is only stationary locally within the moving processing window.
...
If a normal distribution is assumed \citep{Medeiros_1998_IAI}, then:
\begin{equation}
...
If a Gamma distribution is assumed \citep{Lopes_1990_IGARSS}, then: 
\begin{equation}
...
%***IVM, the reference {Medeiros_2003_IntJRemoteSens} could not be found - I got a ? in the PDF....
If a log-normal distribution is assumed \citep{Medeiros_2003_IntJRemoteSens}, then:
\begin{equation}
...
Other distributions that have been proposed include the heavy tailed Rayleigh distribution \citep{Sun_2006_ICVES}, Weibull distribution \citep{Nezry_1997_IGARSS}, beta distribution \citep{Nezry_1997_IGARSS}, Rician distribution \citep{Lewinski_1983_TAntennaPropagation} \dots 
...
In this context, the community appears to focus on using Bayesian inference to choose the most suitable distributions \citep{Walessa_2000_TGRS}.
...
Other approaches that have recently been actively pursued in the research community and in published literature include: 
	...
	wavelets \citep{Gagnon_SPIEProc_1997, VidalPantaleoni_2004_IJRS, Moulin_1993_JMathImageVision, Hervet_1998_SPIE, Hebar_2009_TGRS, Chen_2007_IET, Bianchi_TGRS_2008, Argenti_2006_TGRS}, 
	curvelets \citep{Wang_2007_ICWAPR, Saevarsson_2003_IGARSS, Guo_2008_ICARCV}, 
	contourlets \citep{Foucher_2006_IGARSS}, 
	ridgelets \citep{Saevarsson_2004_IGARSS},
	fuzzy logic \citep{Cheng_ETCS_2009}, 
	anisotropic kernels \citep{DHondt_2006_TGRS}, 
	and autoregressive conditional heteroskedasticity \citep{Amirmazlaghani_2009_DSPWorkshop}. 
...
Not only is statistical analysis important in speckle filtering, the technique is also important for  information extraction \citep{Oliver_1991_JPhysDApplPhys}, target detection \citep{Luttrell_1986_JPhysDApplPhys} and classification \citep{Nyoungui_2002_IntlJRemoteSense}. 
...
Novak \citep{Novak_1990_TAES} pioneered the concept of speckle reduction, and proposed the polarimetric whitening filter (PWF) to produce a single speckle reduced intensity image. 
The main disadvantages of the PWF filter is that polarimetric information is not well-preserved.
Multi-look processing polarimetric results also exhibit bias and distortion \citep{Lee_2008_TGRS}.

Lee \citep{Lee_1991_TGRS} proposed a new filtering algorithm which does suppress speckle for the diagonal elements, however the off-diagonal elements remain un-filtered.
Later, Touzi et. al. \citep{Touzi_1994_TGRS} drew out the principles of speckle filtering of POLSAR data, which they summarised as:
\begin{enumerate}
...
In 1999, Lee \citep{Lee_1999_TGRS} extended his refined-Lee filters to POLSAR data, and argued that all covariance matrix elements should be filtered by the same amount to preserve polarimetric properties. 
...
For instance Martinez et. al.\citep{Lopez_2008_TGRS} proposed a model based polarimetric filter.
(comment: there is no need to quote the year as we are not writing historical events, but rather, should focuse on state of the art techniques)
...
Another different approach is proposed bu Lee \citep{Lee_CRCPress_2009}  to model $S_hS_v^*$.
...
(comment:It is necessary to quote these detail if they are not really useful, and presumably wonít be refer to again in the text? We should try to summarise the essence and shorten the content as mush as possible by not including detail that is not relevant anymore)
...
Again the equations looks overwhelmingly complex, and as admitted by the authors themselves:
	...
	the complex correlation coefficient $\rho$ is not easily estimated \citep{Lee_CRCPress_2009}
...
The main drawback of the approach is that only the intensity was used to determine homogeneity and a primitive averaging filter is performed on off-diagonal elements \citep{Vasile_TGRS_2006}.
...
Vasile, in 2010, published a complicated spherically invariant random vector estimation scheme to estimate the underlying target POLSAR signature \citep{Vasile_2010_TGRS}.
...
In summary, while off-diagonal elements, which carry phase information, are known to be important in interpreting POLSAR data \citep{Rodionova_2009_ESASP}, their estimation and filtering is under-developed in the current research literature. We will discuss our approach ??) based on polarization basis transformation in the next section. (comment: This chapter is about background. So your ìapproachî should not be included in this chapter. From our today conversation, I now know that the various sections are ëassembledí from the various conference papers you published. But you will need to re-arrange the various parts into the appropriate Chapter for the Thesis)   
...
(comment: this section just ësuddenly pop upí here in Chapter 2. You should first describe how it is related to what has been mentioned so far. It is also not mentioned in the beginning of Chapter 2. Although I know what it is about and the reason behind it) 
...
As such, evaluating speckle filters over homogeneous areas has traditionally focussed on evaluating the variation of the estimators'
...
(comment: This is the problem statement of the Thesis, so it should have appeared earlier, somewhere in Chapter 1.)
...
.
 (The above is your proposal. It should not appear here in Chpater 2)One metric that can be used to detect radiometric distortion is the ratio between the estimated and the 
	original value $r = X_{est} / X_{org}$ \citep{Oliver_2004_SciTech,Medeiros_2003_IJRS}.
A somewhat similar metric is used in \citet{Wang_2005_MIPR} as $r_w = (X_{est} - X_{org} )/ X_{org}$.

For homogeneous scenes, Shi et al. \citep{Shi_IGARSS_1994} found that 
	in the original domain the ``standard'' filters (boxcar, Lee \citep{Lee_PAMI_1980}, 
	Kuan \citep{Kuan_1985_PAMI}, Frost\citep{Frost_PAMI_1982}, 
	MAP\citep{Lopes_IGARSS_1990}) and the enhanced version \citep{Lopes_TGRS_1990} can achieve negligible bias. 
...
$ENL=avg(I)^2/var(I)$
that was proposed by Lee \citep{Lee_1981_CGIP}.
Another very similar metric is the ratio of mean to standard deviation, $R=avg(I)/std(I)$ \citep{Gagnon_SPIEProc_1997} 
...
(comment: The above paragrapg should be moved to the beginning of this section to explain why there is a need to evaluate the speckle filter)
...
harder to distinguish than those that are bigger \citep{Medeiros_2003_IJRS}. 
...
Various methods may be applied to extract features (comment: explain first why this is relevant to speckle filter evaluation??); examples of those that have been used are the 
	Hough Transform \citep{Medeiros_2003_IJRS}, Robert gradient edge detector \citep{Gagnon_SPIEProc_1997} 
	and edge map \citep{Frost_PAMI_1982}.
...
Thus Gagnon \citep{Gagnon_SPIEProc_1997} suggested the use of
...
Wang \citep{Wang_2005_MIPR} proposed using fuzzy membership to weight opinions of an expert panel. 
...
Conradsen \citep{Conradsen_2003_TGRS_4} considered the following POLSAR statistics
\begin{equation}
...
This chapter has
  ...
  also illustrates the different published approaches to address the grand challenge of modelling, processing and understanding this type of data.  (Comment: This ësummaryí section is too long. It is to SUMMARIZE the essence of the various points in this chapter, not to described what is to be done in the Thesis. Summary should be one or two paragraph the most.) 
...
Specifically, it showed that when the sense of subtractive distance is not consistent for SAR data, the ratio is proposed as its discrimination measure \citep{Rignot_1993_TGRS_896}.
...
In the next chapter, theoretical models are built to form the foundations required to systematically tackle these issues. 




(Comment: you would need to review the various sections in this chapter, and re-arrange them in a more coherent flow, starting with the basic of SAR and POLSAR, and then bring up the various relevant details. 

If you have a chance to talk to Brain, may be ask him to share with you his introductory chapter, which is written in a very clear manner that explain the issues) 
