...
filtering.
This chapter also shows how the Mean Squared Error (MSE) can then be 
...
\section{
  Clustering and filtering SAR images 
  \footnote{
During the revision of this thesis, Nielsen's work \cite{Nielsen_2012_ICASSP} is brought forward to the author's attention.
...
Assuming prior knowledge of it being homogeneous, thus having a single $\sigma$, the objective is to estimate $\sigma$. 
This section shows how $\sigma$ can be estimated in the log-transformed domain.

It has already been proven (see Eqn. \ref{eqn:SAR Log Intensity PDF}) that the log-transformed dataset $X^l=\log_2 \sigma$ follows a Fisher-Tippet distribution $pdf(x)= \ln 2 \frac{ 2^d }{ e^{2^d} }$, with $d=x-\mu$, and $\mu=2 \log_2 \sigma$. 
...
\lstset{backgroundcolor=\color{darkgray}}
\begin{lstlisting}[frame=tb]
function centers = partition_into_homogeneous_groups(l, threshold)
  ...
  %after this: all paritions are homogeneous
...
function [further_partition_needed, c_new] 
  ...
  all_partitions_homogeneous = true;
  c_new = c;
  for (j = 1 to size_of(c)) 
    g = {L[i] with m[i,j]==1};
    this_partition_homogeneous = 
      (variance(g) < threshold_vector[size_of(g)]);

    if (NOT this_partition_homogeneous) then
      % split non-homogeneous partition
      c_new[j] = min(g);
      c_new[size_of(c_new) + 1] = max(g);

      all_partitions_homogeneous = false;
    end if
  end for

  further_partition_needed = NOT all_partitions_homogeneous;
end function
\end{lstlisting}
...
The ``simulated result'' plot is computed through a Monte Carlo simulation.
And the ``analysis formula'' is a straight forward plotting of the mathematical formula above.
The use of iterative Fuzzy MLE estimations allows further suppression of speckle.
This is illustrated with ``tighter'' distance histograms and reducing Mean Squared Error (MSE), as plotted in Fig. \ref{fig:Homogeneous_Area}
...
%		 \epsffile{images/illustrate_ROC_curve.pdf.s.made.eps} 
                 \plotWithLegend{images/illustrate_ROC_curve.pdf.s.made.eps}{SLC log-intensity} 
...
%		 \epsffile{images/illustrate_ROC_curve.pdf.m.made.eps} 
                 \plotWithLegend{images/illustrate_ROC_curve.pdf.m.made.eps}{SLC log-intensity} 
...
%		 \epsffile{images/log_illustrate_ROC_curve.pdf.s.made.eps} 
                 \plotWithLegend{images/log_illustrate_ROC_curve.pdf.s.made.eps}{SLC log-intensity} 
...
%		 \epsffile{images/log_illustrate_ROC_curve.pdf.m.made.eps} 
                 \plotWithLegend{images/log_illustrate_ROC_curve.pdf.m.made.eps}{9-Look log-intensity} 
...
Fig. \ref{fig:Heterogeneous_Patterns} illustrates the evaluation result in benchmarking the performance of f-MLE filters for artificially-generated heterogeneous test regions.
...
\label{sec:sar_filters_evaluation}
...
Figs. \subref*{fig:hetero_patterns_roc_auc:hist_unfiltered} and \subref*{fig:hetero_patterns_roc_auc:hist_kuan_filtered} 
...
in Figs. \subref*{fig:hetero_patterns_roc_auc:roc_unfiltered} and \subref*{fig:hetero_patterns_roc_auc:roc_kuan_filtered} 
...
In pre-filtered images, shown in Figs. \subref*{fig:hetero_patterns_roc_auc:hist_unfiltered} and 
\subref*{fig:hetero_patterns_mse:unfiltered}, there is no bias error visible.
...
In post-filtered images, shown in Figs. \subref*{fig:hetero_patterns_roc_auc:hist_kuan_filtered} and 
\subref*{fig:hetero_patterns_mse:kuan_filtered}, the situation is more complicated.
Here, besides the effect of the histogram spread, one also needs to take into account the bias error.
A closer examination of Fig. \subref*{fig:hetero_patterns_mse:kuan_filtered} indicates that the output of the 
...
down into the requirements of radiometric preservation and speckle suppression.
In the log-transformed domain, these requirements are equivalent to the bias and variance evaluation of 
...
\label{sec:polsar_speckle_filters_evaluation}
