...
{ Le \MakeLowercase{\textit{et al.}}:  Consistent and Homoskedastic Measures of Distance for Heteroskedastic POLSAR Data}
...
This puts the SAR intensity statistics to lie within the natural coverage of this new theoretical model for POLSAR.
%Thus the SAR intensity statistics can now be considered as a special case of  this new theoretical model for POLSAR.
Not only comprehensive theoretically, the statistical model is also robust in handling practical data. 
...
This section reviews some relevant and published works relating to different measures of distance for POLSAR data. 
...
First is the the following results obtained from the our previous works on single-look SAR \cite{Le_2013_TGRS_SAR_MSE}, i.e. $d=L=1$,
...
And it is shown that: the theoretical model proposed can apply to the practical data, even when this ``imperfection''  are taken into account.
...
The trick is that instead of using the nominal Number of Looks given by the SAR processor,
  an ENL estimation procedure is first undertaken, and then the obtained ENL used subsequently. %the model validation can make use of the Effective Number of Looks that is manually estimated from a given practical dataset.
...
Practically however, the shape of the right-hand-side can be pre-computed
  and for each computed value of $var  \left[ ln|C_v| \right]$, a corresponding value for $L$ can be found by referencing the variance value on the pre-computed graph or by using the following equation, which is extended from a simpler SAR version \cite{Le_2013_TGRS_SAR_MSE} for POLSAR as:  
