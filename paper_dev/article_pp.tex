...
In this section, after the basic foundations of POLSAR statistical analysis is introduced,
  the generic scalar statistical model for the multi-dimensional POLSAR is presented.
...
This claim is extrapolated from the widely-used intensity-ratio as SAR discrimination measure
...
Hence this derivation can be considered
  both as cross-validation for the proposed POLSAR model
  as well as evidence showing that
    the proposed generic model for POLSAR also includes the traditional model for SAR intensity in its natural coverage.
...
\section{Unifying the different discrimination measures proposals for bpth SAR and POLSAR}

At least for both fields of SAR and POLSAR, statistical models have always been the foundation to derive discrimination measures.
For SAR, the statistical model for SAR intensity has been used to derive the most widely used intensity-ratio discrimination measure \cite{Rignot_1993_TGRS_896}.
For POLSAR, the same case should apply, 
  except that so far only asymptotic distribution has been derived for the most common foundation, i.e. the likelihood test statistics.
With the insight gained from section \ref{sec:sar_special_case_of_polsar}, 
  it should be clear that the proposed model can also be used to derive discrimination measures for POLSAR.

As a matter of course, in view of the models given in Eqns \ref{eqn:determinant_distribution},
...
Compared to existing discrimination measures for POLSAR reviewed in section \ref{sec:lit_review}, 
  the proposed dissimilarity measures are much simpler.
They, at the same time, are multi-dimensional extensions of the intensity-ratio discrimination measure widely used in SAR.

Thus the core of the proposed approach is that the determinant of the POLSAR covariance matrix is proposed as the scalar and representative observable of the complex and multi-dimensional data.
%What is the novelty of your solution? What are its advantages
Compared to other published scalar observables with accompanying statistical model,
  the proposed observable and its models are highly representative of the multidimensional data.
The representative power of the proposed observable is justified by the following reasons.
...
In the test, the intensity of single-channel SAR data (HH) for a homogeneous area in both the AIRSAR Flevoland dataset is extracted.
...
The plots are obtained with ENL set an estimated value obtained using the technique presented in \cite{Anfinsen_2009_TGRS_3795}.
In all cases, good visual match is apparent, which hence validates the model proposed above. %providing us a simple validation of the result.
...
From the plots, their visual matches again validates the proposed model. 
...
To begin with a few theoretical properties of the proposed statistical models are discussed.
...
The magnitude however is much more widely used, as it can be shown that it is invariant to a change in the reference frame.
...
This is also similar to the most commonly used magnitude to compare two or more complex numbers. %, most of the times the magnitude is to be used. 
...
(TODO: shorten the above 2 paragraphs. 
...
In this paper,
  ...
  a generic statistical model has been proposed for the POLSAR covariance matrix determinant.
...
The paper also establishes a few POLSAR discrimination measures: the determinant-ratio and the change-ratio,
  which essentially are the generic version of the SAR intensity-ratio.
At the same time, it also shows a perfect match between the (d=1) specific models and the traditional model for SAR intensity,
  effectively encompassing the existing SAR theories within the proposed generic models' natural coverage.

%What are the results? What are their advantages over others' results?
The main result of this paper is the proposal to consider the POLSAR covariance matrix determinant as the scalar and representative observable for the multi-dimensional POLSAR data.
Compared to other published scalar observables, the proposed determinant is highly representative of the multi-dimensional data.
...
The models proposed in this article also has its limitations.
...
%  that is the proposed theoretical model 
  which is only guaranteed to work for homogeneous area.
...
%and hence What can be done next?
  leaving the test of its validity for different types of dataset like bistatic POLSAR data (d=4) or interferrometric POLSAR data (d=6) a possible future work.
