...
Then section \ref{sec:application} presents a simple demonstration where an existing technique of SAR processing is extended for POLSAR data before
...
%In retrospective, intuitively, the concept of the determinant being representative of the POLSAR data can be thought of as being similar to the concept of using the magnitude to represent a complex number
...
While a good p-value does not ensure automatic acceptance of the model,
  consistently high p-values lend high level of confidence to the proposed model\cite{Kolmogoroff_1941_AMS_461}.
...
%\section{Extending Existing SAR Processing Techniques To POLSAR: A Simple Demo}
...
%5Currently while the field of SAR is much more developed than the field of POLSAR,
...
One example has already been presented in this paper,
  where existing SAR discrimination measures are extended towards POLSAR.
This section illustrates another example for a common technique in SAR speckle filters evaluation. 
...
In SAR, the intensity ratio is routinely used in evaluating speckle filters.
...
Since the POLSAR determinant ratio has been shown to be equivalent to the SAR intensity ratio,
  this technique can be equally applied to evaluate POLSAR speckle filters.
To demonstrate this feature, the technique is used to evaluate the performance of 3$\times$3 and 5$\times$5 boxcar POLSAR filters\footnote{While the boxcar filters may not stay in the frontier of POLSAR speckle filtering,
  they are workable here since the focus of this experiment is not on finding the best POLSAR speckle filters available.
  Rather its purpose is to show a quick example of how an existing technique in SAR data processing can be easily adapted for POLSAR data.}.
...
%describe the result
Fig \ref{fig:visual_eval_part_pol_boxcar_speckle_filters_3x3_vs_5x5} shows that the evaluation technique is also workable for POLSAR speckle filters.
In particular, the residual images clearly show both POLSAR speckle filters'  distortion impact.
...
There are many similar SAR data processing techniques that may be adaptable to POLSAR. 
Due to space constraint, these applications, however, fall outside the scope of this paper.
...
Third, this observable also leads to the establishment of new discrimination measures for POLSAR data.
  %, which should increase the usability of this proposal.
And finally, similar to the eigen-values, the determinant of the POLSAR covariance matrix is invariant to any changes in polarization basis.

The detailed and mathematical explanation linking the SAR intensity and the POLSAR determinant has been presented in Section \ref{sec:sar_special_case_of_polsar}.
This paragraph offers another high-level and intuitive linkage.
That is the scalar determinant in POLSAR can be used to represent and compare the multi-dimensonal data,
...
For example, in a polar representation, an extra angle would be requried.
Still, to compare complex numbers, the magnitude is widely used as it can be shown to be invariant to a change in the reference frame.
Similar to the maginute, the determinant of the POLSAR covariance matrix is also scalar and invariant to a change of polarization basis.
%while it may not be \textit{fully} representative of the POLSAR data, 
Thus the scalar observable is proposed to represent and distinguish the multi-dimensional dataset.
...
Two example application have already been presented in this paper. 
  In the first instance, existing SAR discrimination measures are extended towards POLSAR.
  While in the second case, existing technique to evaluate SAR speckle filters is also shown extensible towards POLSAR.
